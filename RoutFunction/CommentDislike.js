...
async function CommentDislike(req, res) {
  try {
    ...
    if (postExist.length > 0) {
      ...
      if (inlike.length > 0) {
        await dbConnection.query('DELETE FROM comment_like WHERE comment_slno = ? AND user_email = ?', [postno, userMail[0].email]);
        await dbConnection.query('INSERT INTO comment_dislike (comment_slno, user_email) VALUES (?, ?)', [postno, userMail[0].email]);
        ...
        const [like] = await dbConnection.query('SELECT dislike_amount FROM comments WHERE slno = ?', [postno]);
        await dbConnection.query('UPDATE comments SET dislike_amount = ? WHERE slno = ?', [like[0].dislike_amount + 1, postno]);
