...
async function PostDislike(req, res) {
  try {
    ...
    if (postExist.length > 0) {
      ...
      if (inlike.length > 0) {
        await dbConnection.query('DELETE FROM post_like WHERE post_slno = ? AND user_email = ?', [postno, userMail[0].email]);
        await dbConnection.query('INSERT INTO post_dislike (post_slno, user_email) VALUES (?, ?)', [postno, userMail[0].email]);
        ...
        if (author[0].user_slno != usersl) {
          await dbConnection.query('INSERT INTO notification (owner_slno, commenter_slno, commenter_email, reason, post_slno) VALUES (?, ?, ?, ?, ?)', [author[0].user_slno, usersl, commenter[0].email, 'Disliked Your Post.', postno]);
        }

        return res.status(200).json({
          message: 'Success'
        });
      } else if (inDislike.length > 0) {
        return res.status(200).json({
          message: 'Failed'
        });
      } else {
        ...
        const [like] = await dbConnection.query('SELECT dislike_amount FROM posts WHERE slno = ?', [postno]);
        await dbConnection.query('UPDATE posts SET dislike_amount = ? WHERE slno = ?', [like[0].dislike_amount + 1, postno]);
